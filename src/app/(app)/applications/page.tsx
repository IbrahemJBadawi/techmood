    ...
    <>
      <section className="section-block">
        <div className="row-between">
          <div>
            <h2 style={{ fontSize: '1.2rem' }}>طلباتي</h2>
            <p className="muted" style={{ fontSize: '0.9rem', marginTop: 6 }}>
              {open.length} طلب قيد النظر من أصل {applications?.length ?? 0}.
            </p>
          </div>
          <Link className="btn btn-ghost btn-sm" href="/marketplace">تصفّح السوق</Link>
        </div>
      </section>

      {(applications?.length ?? 0) === 0 ? (
        <p className="notice">لم تقدّم على أي فرصة بعد.</p>
      ) : (
        <table className="data">
          <thead>
            <tr><th>الفرصة</th><th>النوع</th><th>الأجر</th><th>الحالة</th><th>التاريخ</th><th></th></tr>
          </thead>
          <tbody>
            {applications!.map((application) => {
              ...
              return (
                <tr key={application.id}>
                  ...
                  <td>{opportunity ? OPPORTUNITY_KIND[opportunity.kind].label : '—'}</td>
                  <td className="eng">{opportunity ? compensationLabel(opportunity) : '—'}</td>
                  <td><span className={`status-pill ${stage.className}`}>{stage.text}</span></td>
                  <td className="eng">{new Date(application.created_at).toLocaleDateString('ar-EG')}</td>
                  <td>
                    {opportunity && (
                      <Link className="btn btn-ghost btn-sm" href={`/marketplace/${opportunity.id}`}>عرض</Link>
                    )}
                  </td>
                </tr>
              );
