    ...
    <>
      <section className="section-block">
        <div className="row-between">
          <h2 style={{ fontSize: '1.2rem' }}>طلبات الحاضنة</h2>
          <Link className="btn btn-ghost btn-sm" href="/incubator">الحاضنة</Link>
        </div>
        <p className="muted" style={{ fontSize: '0.9rem', marginTop: 6 }}>
          كل طلب يحمل المرحلة ونسبة اكتمال خطة العمل وقت التقديم — حتى تُقارن الطلبات بما كان
          فعلاً، لا بما صار بعدها.
        </p>
      </section>
      ...
      <section className="section-block">
        ...
        {waiting.length === 0 ? (
          <p className="notice">لا طلبات بانتظار المراجعة 🎉</p>
        ) : (
              ...
              <article className="panel section-block" key={application.id}>
                <div className="row-between" style={{ alignItems: 'flex-start' }}>
                  ...
                  <div className="tags-row">
                    <span className="badge-pill">
                      {STARTUP_STAGES.find((stage) => stage.key === application.stage_at_application)?.label ?? '—'}
                    </span>
                    <span className="badge-pill eng">خطة {application.plan_percent_at_application ?? 0}%</span>
                  </div>
                </div>
                ...
                {startup?.problem_ar && (
                  <div style={{ marginTop: 12 }}>
                    <p className="muted" style={{ fontSize: '0.78rem' }}>المشكلة</p>
                    <p style={{ fontSize: '0.86rem' }}>{startup.problem_ar}</p>
                  </div>
                )}

                <div style={{ marginTop: 12 }}>
                  <p className="muted" style={{ fontSize: '0.78rem' }}>طلبه</p>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 16, flexWrap: 'wrap' }}>
                  <form action={reviewApplication}>
                    ...
                    <button className="btn btn-primary btn-sm">اقبل في الحاضنة</button>
                  </form>
                  <form action={reviewApplication} style={{ display: 'flex', gap: 8, flex: 1, minWidth: 260 }}>
                    <input type="hidden" name="application_id" value={application.id} />
                    <input type="hidden" name="decision" value="reject" />
                    <input name="note" required placeholder="سبب الرفض — يظهر للمؤسس" style={{ flex: 1, minWidth: 0 }} />
      ...
      {settled.length > 0 && (
        <section className="section-block">
          <h3 style={{ fontSize: '1rem', marginBottom: 12 }}>سجلّ الطلبات</h3>
          <table className="data">
            <thead><tr><th>المشروع</th><th>المرحلة وقت التقديم</th><th>الحالة</th><th>ملاحظة</th></tr></thead>
            <tbody>
