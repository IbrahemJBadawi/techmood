  ...
  return (
    <>
      <section className="section-block row-between">
        <div>
          <h1 style={{ fontSize: '1.2rem', marginBottom: 4 }}>{t('طلبات الأدوار', 'Role requests')}</h1>
          <p className="muted" style={{ fontSize: '0.88rem' }}>
            {t('رفض الطلب لا يُلغي الحساب — الشخص يبقى طالباً ويستطيع التقدّم مرة أخرى.', 'Turning a request down does not close the account — the person stays a student and can apply again.')}
          </p>
        </div>
        <nav className="tags-row">
          {[
            { key: undefined, label: t('المفتوحة', 'Open') },
            { key: 'pending_review', label: t('قيد المراجعة', 'Pending review') },
            { key: 'needs_more_info', label: t('بانتظار المتقدّم', 'Waiting on applicant') },
      ...
      {rows.length === 0 && <p className="panel muted">{t('لا طلبات في هذه القائمة.', 'Nothing in this list.')}</p>}
            ...
            <article className="panel" key={row.id}>
                ...
                <div>
                  <h2 style={{ fontSize: '1rem' }}>
                    ...
                    <span className="muted" style={{ fontWeight: 400 }}>
                      {t(' يطلب دور ', ' is asking for the ')}{t(roleLabel(row.role as UserRole))}
                      {t('', ' role')}
                    </span>
                  </h2>
                  <p className="muted" style={{ fontSize: '0.82rem' }}>
                    ...
                    {applicant?.country ? t(`، ${applicant.country}`, `, ${applicant.country}`) : ''}
                  </p>
                </div>
                <span className={`pill pill-${ROLE_STATUS_TONE[status]}`}>
                  {t(ROLE_STATUS_LABEL[status])}
                </span>
              </div>
              ...
              {mentor && (
                <dl className="summary-list">
                  <div><dt>{t('سنوات الخبرة', 'Years of experience')}</dt><dd>{mentor.years_experience ?? '—'}</dd></div>
                  <div><dt>{t('ساعات أسبوعية', 'Hours a week')}</dt><dd>{mentor.weekly_hours ?? '—'}</dd></div>
                  <div><dt>{t('مجالات الإرشاد', 'Mentoring fields')}</dt><dd>{mentor.domains?.join(t('، ', ', ')) || '—'}</dd></div>
                  <div><dt>{t('لغات الجلسات', 'Session languages')}</dt><dd>{mentor.languages?.join(t('، ', ', ')) || '—'}</dd></div>
                  <div><dt>{t('الخبرة', 'Experience')}</dt><dd>{mentor.experience_ar ?? '—'}</dd></div>
                  <div><dt>{t('الدافع', 'Motivation')}</dt><dd>{mentor.motivation_ar ?? '—'}</dd></div>
                  {mentor.linkedin_url && (
              ...
              {row.review_note && status === 'needs_more_info' && (
                <p className="notice">{t('طُلب منه: ', 'Asked of them: ')}{row.review_note}</p>
              )}
